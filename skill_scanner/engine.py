...
def scan_skill(
    ...
    start = time.time()
    all_findings = []
    scanner_results = {}  # track which scanners ran and their real passed count

    # Track real scanner invocations
    scanners = {
    ...
    for name, scanner_fn in scanners.items():
        ...
        # Count real passed/info findings from this scanner
        passed = len([f for f in findings if f["severity"] in ("passed", "info")])
        scanner_results[name] = {"checks": len(findings), "passed": passed}

    # Diff mode: compare against baseline via trace engine
    diff_result = None
    if diff_mode and baseline_dir:
        try:
            ...
            pass  # diff is optional enhancement
    ...
    return {
        ...
        "total": total,
        "blocked": blocked,
        "blocked_rules": list(set(f["rule"] for f in all_findings if f.get("action") == "block")),
...
def _build_baseline_trace(recorder: TraceRecorder, directory: str, is_current: bool = False):
    """Build a simple trace from scanning file metadata for baseline comparison."""
    import json, yaml
    base = Path(directory)
    skill_path = base / "SKILL.md"
    if not skill_path.exists():
